 ...
 * java.lang.SecurityException: getSerial: The user 10197 does not meet the requirements to access device identifiers.
 */
@get:RequiresPermission(permission.READ_PHONE_STATE)
@get:SuppressLint("HardwareIds")
val serial: String
 ...
 *
 * Must hold `<uses-permission android:name="android.permission.READ_PHONE_STATE" />`
 * java.lang.SecurityException: getSubscriberId: The user 10197 does not meet the requirements to access device identifiers.
 */
@get:RequiresPermission(permission.READ_PHONE_STATE)
@get:SuppressLint("HardwareIds")
val iMSI: String
 ...
 * @return 联系人链表
 */
@RequiresPermission(allOf = [permission.READ_EXTERNAL_STORAGE, permission.READ_CONTACTS])
fun getAllContactInfo(): List<HashMap<String, String>> {
    ...
    val dateUri = Uri.parse("content://com.android.contacts/data")
    ...
    cursor.use {
        ...
        if (it != null) {
            while (it.moveToNext()) {
                ...
                // getColumnIndex: 查询字段在cursor中索引值,一般都是用在查询字段比较多的时候
                // 判断contact_id是否为空
                if (contactId.isNotEmpty()) { //null   ""
                    ...
                    val c = resolver.query(
                        dateUri, arrayOf("data1", "mimetype"), "raw_contact_id=?",
 ...
 *
 * java.lang.SecurityException: getDeviceId: The user 10197 does not meet the requirements to access device identifiers.
 */
@RequiresPermission(permission.READ_PHONE_STATE)
